...
async def check_and_remove_role(member: discord.Member, role_name: str) -> int | None:
    ...
    If yes - it removes the role and returns the full price of the role.
    If no - func does nothing.

    :param member: Discord member to check.
    :param role_name: The name of the role we are looking for.

    :return: If `role_name` is found and can be removed, it returns the full price amount.
             If `role_name` is not in the price dictionaries, returns -1.
